...
var Blockchain = module.exports = function (opts) {
  ...
  this.store.get(this.genesis.hash, function (err, block) {
    if (err && !err.notFound) return self._error(err)
    if (block) {
      self.initialized = true
      self.emit('ready')
      return
    }
    if (self.store.isClosed()) return
    self.store.put(self.genesis, function (err) {
...
Blockchain.prototype._findDownloadPeer = function (cb) {
  ...
  var peers = this.peerGroup.peers
...
Blockchain.prototype._onHeaders = function (message) {
  ...
  this.processHeaders(message.headers, function (err, last) {
    if (err) {
      return self._error(new Error('Peer sent invalid headers:' + err + ' ' + last))
    }
    if (self.syncing) {
      self.emit('sync', last)
      self._getHeaders()
