...
Blockchain.prototype.sync = function(opts, cb) {
  ...
  var syncDone = function() {
    if(self.downloadPeer && self.downloadPeer.startHeight
    && self.tip.height >= self.downloadPeer.startHeight) {
...
Blockchain.prototype._processHeader = function(prev, header, cb) {
  ...
  if(self.shouldRetarget(height)) {
    return self.calculateTarget(block, function(err, expected) {
      if(err) return cb(err, block);

      if(toCompactTarget(expected) !== header.bits) {
        return cb(new Error('Target in block (' + actual.toString(16) + ')' +
