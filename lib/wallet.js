...
Wallet.prototype._listenToChain = function () {
  ...
  if (this.peers.numberConnected() === 0) {
    this.peers.on('peerconnect', function () { self._listenToChain() })
    return
  }
...
Wallet.prototype._processTransaction = function (tx) {
  ...
  for (var i = this.state.gen; i < this.state.i + this.lookAhead; i++) {
    (function (i) {
      tasks.push(function (cb) {
        ...
        console.log(key.publicKey.toBuffer())
